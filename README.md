# Client–Server Benchmark Foundation (TypeScript)
...
- `ui/web`: Vite + React + TypeScript + TailwindCSS + shadcn/ui frontend with 3 behaviors:
...
- Node.js (use latest stable via `nvm` if available)
...
- `DEMO_FREEZE_BOTS=1 npm run dev` — run in demo mode but freeze server bots (useful for e2e)
- `npm run dev:server`
- `npm run dev:web`
- `npm run build`
- `npm run test`
